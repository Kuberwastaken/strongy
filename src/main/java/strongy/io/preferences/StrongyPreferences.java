package strongy.io.preferences;

import com.github.kwhat.jnativehook.keyboard.NativeKeyEvent;
import com.sun.jna.Platform;
import strongy.io.KeyConverter;
import strongy.io.preferences.enums.AllAdvancementsToggleType;
import strongy.io.preferences.enums.AngleAdjustmentDisplayType;
...
public class StrongyPreferences {
	...
	public final BooleanPreference multiStrongholdTracking;
	...
	public StrongyPreferences(IPreferenceSource source) {
		this.source = source;
		// Integer
		...
		informationCombinedCertaintyEnabled = new BooleanPreference("combined_offset_information_enabled", true,
				source);
		informationPortalLinkingEnabled = new BooleanPreference("portal_linking_warning_enabled", true, source);
		multiStrongholdTracking = new BooleanPreference("multi_stronghold_tracking", false, source);
		// String
		...
		// Multiple choice
		size = new MultipleChoicePreference<>("size", SizeSetting.SMALL, new int[] { 0, 1, 2 },
				new SizeSetting[] { SizeSetting.SMALL, SizeSetting.MEDIUM, SizeSetting.LARGE }, source);
		strongholdDisplayType = new MultipleChoicePreference<>("stronghold_display_type",
				StrongholdDisplayType.FOURFOUR, new int[] { 0, 1, 2 },
				new StrongholdDisplayType[] { StrongholdDisplayType.FOURFOUR, StrongholdDisplayType.EIGHTEIGHT,
						StrongholdDisplayType.CHUNK },
				source);
		view = new MultipleChoicePreference<>("view", MainViewType.BASIC, new int[] { 0, 1 },
				new MainViewType[] { MainViewType.BASIC, MainViewType.DETAILED }, source);
		mcVersion = new MultipleChoicePreference<>("mc_version", McVersion.PRE_119, new int[] { 0, 1 },
				new McVersion[] { McVersion.PRE_119, McVersion.POST_119 }, source);
		allAdvancementsToggleType = new MultipleChoicePreference<>("aa_toggle_type",
				AllAdvancementsToggleType.Automatic, new int[] { 0, 1 }, new AllAdvancementsToggleType[] {
						AllAdvancementsToggleType.Automatic, AllAdvancementsToggleType.Hotkey },
				source);
		defaultBoatType = new MultipleChoicePreference<>("default_boat_type", DefaultBoatType.GRAY,
				new int[] { 0, 1, 2 },
				new DefaultBoatType[] { DefaultBoatType.GRAY, DefaultBoatType.BLUE, DefaultBoatType.GREEN }, source);
		angleAdjustmentType = new MultipleChoicePreference<>("angle_adjustment_type", AngleAdjustmentType.SUBPIXEL,
				new int[] { 0, 1, 2 },
				new AngleAdjustmentType[] { AngleAdjustmentType.SUBPIXEL, AngleAdjustmentType.TALL,
						AngleAdjustmentType.CUSTOM },
				source);
		angleAdjustmentDisplayType = new MultipleChoicePreference<>("angle_adjustment_display_type",
				AngleAdjustmentDisplayType.ANGLE_CHANGE, new int[] { 0, 1 },
				new AngleAdjustmentDisplayType[] { AngleAdjustmentDisplayType.ANGLE_CHANGE,
						AngleAdjustmentDisplayType.INCREMENTS },
				source);
		...
		Assert.isTrue(settingsVersion.get() >= 3); // Do >= to allow users to downgrade to an earlier version, in case
													// newer version has issues
	}
