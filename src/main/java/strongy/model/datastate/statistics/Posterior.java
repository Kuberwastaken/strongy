...
public class Posterior {
	...
	public Posterior(IReadOnlyList<IEnderEyeThrow> eyeThrows, IDivineContext divineContext,
			StandardDeviationSettings standardDeviationSettings, boolean useAdvStatistics, McVersion version) {
	...
	public Chunk getMostProbableChunk() {
		return chunks.stream()
				...
				.orElseThrow(() -> new IllegalStateException("No chunks in posterior distribution"));
	}
	...
	private void updateConditionalProbability(Chunk chunk, IEnderEyeThrow t) {
		...
		double v2 = getVarianceFromPositionImprecision(deltax * deltax + deltaz * deltaz, t.xInOverworld(),
				t.zInOverworld());
		double v = s1 * s1 + v2;
	...
	private double getVarianceFromPositionImprecision(double distance2, double throwX, double throwZ) {
		...
		if ((Math.abs(throwX - Math.floor(throwX) - 0.3) < 1e-6 || Math.abs(throwX - Math.floor(throwX) - 0.7) < 1e-6)
				&&
				(Math.abs(throwZ - Math.floor(throwZ) - 0.3) < 1e-6
						|| Math.abs(throwZ - Math.floor(throwZ) - 0.7) < 1e-6)) {
			return 0;
		}
	 ...
	 */
	private void closestStrongholdCondition(IEnderEyeThrow t, double probabilityThreshold) {
		// Sort by weight descending
		chunks.sort((c1, c2) -> -Double.compare(c1.weight, c2.weight));

		// Determine how many chunks to compute exactly
		int exactCount = 0;
		for (int i = 0; i < chunks.size(); i++) {
			if (i < 100 || chunks.get(i).weight > probabilityThreshold) {
				exactCount++;
			} else {
				break;
			}
		...
		// Parallel exact computation for high-probability chunks
		List<Chunk> exactChunks = chunks.subList(0, exactCount);
		double[] probs = exactChunks.parallelStream()
				.mapToDouble(c -> closestStrongholdCondition(c, t))
				.toArray();

		double avgProb = 0;
		for (double p : probs)
			avgProb += p;
		avgProb /= probs.length;

		// Approximate remaining chunks with the average probability
		for (int i = exactCount; i < chunks.size(); i++) {
			chunks.get(i).weight *= avgProb;
		}
