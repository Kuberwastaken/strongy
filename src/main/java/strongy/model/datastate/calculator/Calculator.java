...
import strongy.model.datastate.statistics.Prior;
...
public class Calculator implements ICalculator {
	...
	private final boolean multiStrongholdTracking;
	...
	public Calculator(CalculatorSettings calculatorSettings, StandardDeviationSettings standardDeviationSettings) {
		...
		this.multiStrongholdTracking = calculatorSettings.multiStrongholdTracking;
	}

	@Override
	public ICalculatorResult triangulate(IReadOnlyList<IEnderEyeThrow> eyeThrows,
			IObservable<IPlayerPosition> playerPos, IDivineContext divineContext) {
		if (eyeThrows.size() == 0)
			return null;
		long t0 = System.currentTimeMillis();
		// Calculate posteriors
		Posterior posterior = new Posterior(eyeThrows, divineContext, standardDeviationSettings, useAdvancedStatistics,
				mcVersion);
		Logger.log("Time to triangulate: " + (System.currentTimeMillis() - t0) / 1000f + " seconds.");
		if (multiStrongholdTracking) {
			return new MultiRingCalculatorResult(posterior, eyeThrows, playerPos, numberOfReturnedPredictions,
					mcVersion);
		}
	...
	/**
	 * Triangulate with multi-ring tracking. Returns per-ring predictions.
	 */
	public ICalculatorResult triangulateMultiRing(IReadOnlyList<IEnderEyeThrow> eyeThrows,
			IObservable<IPlayerPosition> playerPos, IDivineContext divineContext) {
		if (eyeThrows.size() == 0)
			return null;
		long t0 = System.currentTimeMillis();
		Posterior posterior = new Posterior(eyeThrows, divineContext, standardDeviationSettings, useAdvancedStatistics,
				mcVersion);
		Logger.log("Time to triangulate (multi-ring): " + (System.currentTimeMillis() - t0) / 1000f + " seconds.");
		return new MultiRingCalculatorResult(posterior, eyeThrows, playerPos, numberOfReturnedPredictions, mcVersion);
	}
